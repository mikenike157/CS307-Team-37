...
io.sockets.on('connection', function (socket) {
    ...
    if (data == "/start") {
      ...
      if (counter == players.length-1) {
        for (var i = 0; i < players.length; i++) {
          if (players[i].state != "FOLDED") {
            players[i].chips += currentPot;
            io.sockets.in(socket.room).emit('updatechat', "Server", players[i].username + " won. They win " + currentPot + " chips.");
            beginRound();
          }
        }
      }
    ...
    console.log(players)
    console.log("SHOULD GET TO PRINTINFO")
...
function beginRound() {
  ...
  for (var i = 0; i < availSockets.length; i++) {
    ...
    io.to(availSockets[i]).emit('updatechat', "Your Cards: ", fixedPCards[i]);
  }
...
function progressGame(socket) {
  if (gameState == 0) {
    ...
    currentBet = 0;
    for (var i = 0; i < players.length; i++) {
      players[i].order = (players[i].order + 1) % players.length;
    }
